...
describe('CarouselView snap scroll parity', () => {
  it('sets decelerationRate="fast" on horizontal ScrollView (RN snap scroll easing parity)', async () => {
    const layer: CarouselLayer = {
      id: 'lyr_car_snap',
      kind: 'carousel',
      slides: [
        {
    ...
    await act(async () => {
      tree = TestRenderer.create(
        createElement(CarouselView, { layer, ctx: smokeCtx(), renderLayer: noopRender }),
      );
    });
    const scrollViews = tree!.root.findAllByType('ScrollView');
    expect(scrollViews).toHaveLength(1);
    expect(scrollViews[0]?.props.decelerationRate).toBe('fast');
    tree?.unmount();
  });
});

describe('CarouselView brand gradient overflow clip', () => {
  it('sets overflow hidden on outer chrome when background is a brand gradient', async () => {
    ...
    await act(async () => {
      tree = TestRenderer.create(
        createElement(CarouselView, { layer, ctx: gradientCtx(), renderLayer: noopRender }),
      );
    });
