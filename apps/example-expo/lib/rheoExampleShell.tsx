...
import { RheoProvider, type RheoConfig } from '@getrheo/react-native-expo';
import {
  createContext,
  useCallback,
  useContext,
...
type ExampleRheoContextValue = {
  /** Re-read AsyncStorage and mount/update the root `RheoProvider`. */
  reloadFromStorage: () => Promise<void>;
  /** Apply in-memory config (e.g. right after the user taps Start flow). */
  syncFromSavedConfig: (saved: SavedConfig) => void;
  /** Drop the root provider after reset. */
  clearShell: () => void;
};
